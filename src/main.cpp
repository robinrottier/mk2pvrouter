...
enum loadStates {LOAD_ON, LOAD_OFF}; // all loads are logically active-low
...
//  digitalWrite (outOfRangeIndication, LED_OFF); 
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      Mk2_fasterControl_withRF_2.ino");
  Serial.println();
...
void allGeneralProcessing()
{
  ...
  //
  const float lpf_gain = 12;  // <- setting this to 0 disables this extra processing 
...
*/  
        ...
        { 
          ...
          if (RF_send_counter >= RF_SEND_PERIOD)
          {
            ...
            tx_data.powerAtSupplyPoint_Watts = realPower_long * powerCal_grid;
            ...
            tx_data.divertedPower_Watts = realPower_long * powerCal_diverted;
            tx_data.Vrms_times100 = 
              (int)(100 * voltageCal * sqrt(sum_Vsquared / samplesDuringDatalogPeriod));           
...
/*             
            ...
            Serial.print(",  "); 
            Serial.print(tx_data.divertedPower_Watts); 
            Serial.print(",  "); 
    ...
    if(sampleSetsDuringThisMainsCycle == 5) // to distribute the workload within each mains cycle
    {
      ...
      {           
        ...
        if (recentTransition)
        {
          ...
          if (energyInBucket_long < workingEnergyThreshold_lower)
          {
  ...
  {     
    ...
    if (sampleSetsDuringNegativeHalfOfMainsCycle == 5) 
    {
      ...
      {           
        ...
        if (energyInBucket_prediction < workingEnergyThreshold_lower)        
        { 
