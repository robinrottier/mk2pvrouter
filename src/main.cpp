...
void setup()
{
  ...
  Serial.print("powerCal_CT1, for grid consumption =      ");
  Serial.println(powerCal_grid, 4);
  Serial.print("powerCal_CT2, for diverted power  =      ");
  Serial.println(powerCal_diverted, 4);
  Serial.print("voltageCal, for Vrms  =      ");
  Serial.println(voltageCal, 4);
 ...
 */
{
  ...
  case 1:
    ...
    phaseShiftedSampleVminusDC = lastSampleVminusDC_long + (((sampleVminusDC_long - lastSampleVminusDC_long) * phaseCal_grid_int) >> 8);
    //
  ...
  case 2:
    ...
    phaseShiftedSampleVminusDC = lastSampleVminusDC_long + (((sampleVminusDC_long - lastSampleVminusDC_long) * phaseCal_diverted_int) >> 8);
    //
    // calculate the "real power" in this sample pair and add to the accumulated sum
    filtV_div4 = phaseShiftedSampleVminusDC >> 2; // reduce to 16-bits (now x64, or 2^6)
    filtI_div4 = sampleIminusDC >> 2;             // reduce to 16-bits (now x64, or 2^6)
